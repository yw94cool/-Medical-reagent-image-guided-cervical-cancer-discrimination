...
def crack_captcha(captcha_image, numberOfImage, modelfile, IMAGE_WIDTH, IMAGE_HEIGHT):
    ...
    with tf.Session() as sess:
        ...
        yin = 0
        yisi = 0
        yinerror, yangerror, yisierror = 0, 0, 0
        for n in range(numberOfImage):
            ...
            if label == text:
                nn += 1
            elif label == "yin" and text != "yin":
                yinerror += 1
            elif label == "yang" and text != "yang":
                yangerror += 1
            elif label == "yisi" and text != "yisi":
                yisierror += 1
        ...
        print("正确率：",nn/numberOfImage, "阴性正确率：",(yin-yinerror)/yin , "阳性正确率：",(yang-yangerror)/yang ,"疑似正确率：",(yisi-yisierror)/yisi)
